...
const AppointmentCard = ({ 
  ...
  const paymentStatusConfig = {
    pendente: { label: 'Pagamento pendente', variant: 'secondary' },
    pago: { label: 'Pagamento recebido', variant: 'default' },
    reembolsado: { label: 'Pagamento reembolsado', variant: 'outline' },
  };

  const paymentConfig = paymentStatusConfig[appointment.statusPagamento?.toLowerCase?.() || ''] || {
      ...
      <CardContent className="p-6">
        ...
        <div className="flex items-center justify-between mb-4 p-3 bg-white/50 rounded-lg border border-gray-100">
          <div className="flex items-center space-x-4">
            ...
            {appointment.valorPago && Number(appointment.valorPago) > 0 && (
              <div className="flex items-center text-sm text-gray-600">
                <DollarSign className="w-4 h-4 mr-1 text-emerald-500" />
                <span className="font-medium">
                  Pago: {formatPrice(Number(appointment.valorPago))}
                </span>
              </div>
            )}
          </div>
