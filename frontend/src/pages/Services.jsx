    ...
    const handleCreateService = async (e) => {
        ...
        if (isPaidService) {
            ...
            if (Number.isNaN(parsedPrice) || parsedPrice <= 0) {
                toast.error('Informe um valor válido para serviços pagos.');
                setSubmitting(false);
                return;
            }
            serviceData.preco = parsedPrice;
        }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                            ...
                            <div className="flex items-center space-x-3">
                                <Switch
                                    ...
                                    onCheckedChange={setIsPaidService}
                                />
                                <div>
                                    <Label htmlFor="servico-pago" className="mb-1 block">Serviço pago?</Label>
                                    <p className="text-sm text-gray-500">
                                        Ative para definir um valor que será cobrado dos clientes.
                                    </p>
                                </div>
                            </div>
                            {isPaidService && (
                            ...
                            {services.map((service) => (
                                <div key={service.id} className="flex items-center justify-between p-3 border-b last:border-b-0 hover:bg-gray-50 rounded-md">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500">
                                            Duração: {formatarMinutosParaDuracao(service.duracao_minutos)}{' '}
                                            {parseFloat(service.preco || 0) > 0
